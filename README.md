...
无参数 Run 的父范围始终是整个仓库/系统，当前目录、缺陷类型或修复批次只是发现簇，不能反过来缩小 Run。Agent 持续维护并全局比较三条候选主线：

1. 用户与业务结果；
2. 工程与可靠性；
3. 架构与持续演进。

连续选择同一问题族前，必须证明它仍比其他主线候选更重要。可安全运行的用户界面必须用浏览器遍历代表性关键旅程，并在用户可见修复后复验；静态代码和组件测试不能替代运行态 UX 证据。

`safe work exhausted` 不是“当前搜索没结果”，而是完成证明：最后一次实质修复后重新覆盖三主线、主动搜索当前模块/分类/测试模式之外的反例、核对开放债务和能力/规则分裂，并让 schema-v3 `RUN-*` 通过 `validate_run_completion.py`。覆盖矩阵、聚合验证、完成挑战和终止原因不得保留 `TBD` 占位证据。验证未通过只能记为 `partial`、`blocked` 或 `interrupted`，不能完成 Run 或宿主持久 Goal。
...
- 每个语义批次都必须完成风险所需验证；`RUN-*` 是连续序列的权威账本，仅在风险、漂移恢复、兼容分阶段、仓库规则或复杂交接需要时创建独立 `BATCH-*`。普通 Checkpoint 后自动选择下一项工作。
...
真实停止条件只有：完成证明确认三主线和运行态 UX 范围内没有可修复问题；剩余工作全部缺业务权威、审批、证据、环境或专项能力；需要受保护外部操作或 R3/R4 决策；并行漂移使候选工作不安全；同一假设失败三次后该假设被隔离且没有其他安全工作；或宿主中断、挂起、限流、结束任务。除宿主中断外，声明 `completed` 还必须通过 Run Completion Validator。
...
每个修改批次都要求基线、回归测试、分层验证、最终 Diff/调用方检查、架构复扫和可恢复 Checkpoint。同一输入指纹下可复用昂贵验证证据；Revision、命令、环境、依赖、输入或受影响路径变化时必须重跑风险所需门禁。同一修复假设连续失败三次后停止，不进行第四次盲改。
